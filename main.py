...
class DataHandler:
    ...
    def optimization(self, reward):
        return (
            self.steps_done % params.K_FRAME == 0 and reward
        )  # or reward in (-10, 50, 200)

    def avoid_beginning_steps(self):
        for i_step in range(params.AVOIDED_STEPS):
            obs, reward, done, info = self._step(3)

    def select_action(self, state):
        sample = random.random()
        eps_threshold = max(
            EPS_MIN, EPS_MAX - (EPS_MAX - EPS_MIN) * self.learn_counter / EPS_DECAY
        )
        self.steps_done += 1
        with torch.no_grad():
    ...
    def run(self):
        progress = Progress(
            ...
            BarColumn(),
            TextColumn(
                "[progress.percentage]{task.completed:,d}/{task.total:,d} \[{task.percentage:>3.0}%]"
            ),
        )
    ...
    def run_one_episode(self):
        ...
        while True:
            ...
            obs, reward_, done, info = self._step(action_)
...
if __name__ == "__main__":
    ...
    datahandler = DataHandler(
        env, policy, target, memory, buffer, paths, save=not args.stream
    )
    if args.stream:
